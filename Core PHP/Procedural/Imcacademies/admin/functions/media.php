...
if(isset($_POST) && isset($_POST['add'])) {
    ...
    $sql = "insert into `media`(`user_id`,`name`,`link`,`type`,`image`) values('$user_id','$name','$link','$type','$filename')";
    mysqli_query($db->conn, $sql);

    // print_r(mysqli_error($db->conn));die;

    header("Location: ../src/media.php");
}
...
if(isset($_POST) && isset($_POST['update'])) {
    ...
    $sql = "update `media` set `name`='$name', `type`='$type', `link`='$link'";
    if(!is_null($filename)) {
        $sql .= ",`image`='$filename'";
    }
    $sql .= "where `id`='$id'";
