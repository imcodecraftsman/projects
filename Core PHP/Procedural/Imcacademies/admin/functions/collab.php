...
if(isset($_POST) && isset($_POST['add'])) {
    ...
    $sql = "insert into `collaboration`(`user_id`,`name`,`link`,`type`,`image`) values('$user_id','$name','$link','$type','$filename')";
    mysqli_query($db->conn, $sql);

    // print_r(mysqli_error($db->conn));die;

    header("Location: ../src/collaboration.php");
}
...
if(isset($_GET['cid'])) {
    ...
    $id = $_GET['cid'];
    $img = $_GET['img'] ?? null;
    $sql = "delete from `collaboration` where id='$id'";
    mysqli_query($db->conn, $sql);
...
if(isset($_POST) && isset($_POST['update'])) {
    ...
    $sql = "update `collaboration` set `name`='$name', `type`='$type', `link`='$link'";
    if(!is_null($filename)) {
        $sql .= ",`image`='$filename'";
    }
    $sql .= "where `id`='$id'";
