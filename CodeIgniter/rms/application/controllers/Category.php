...
class Category extends Admin_Controller 
{
	...
	public function fetchCategoryData()
	{
		...
		foreach ($data as $key => $value) {
			...
			$status2 = ($value['main_category'] == 'inventory') ? '<span class="label label-success">Active</span>' : '<span class="label label-danger">Inactive</span>';
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	$data = array(
				...
				'main_category' => $this->input->post('maincategory'),
		...
		if($id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
					...
					'main_category' => $this->input->post('editmaincategory'),
